...
import { guessChatLlm } from '@/utils/llm';
...
const onSetCurrentAsDefault = async () => {
  await callLlmActive('chat', currentLlm.name);
...
const selectFirstLlm = () => {
  const llms = llmStore.listByType('chat');
  if (llms.length > 0) {
...
<template>
  <div class="flex h-full w-full">
    <div class="border-l-1 flex flex-col gap-2 border-neutral-100 bg-white p-4">
      <el-button class="mb-2 min-w-48" round @click="state.showAddChatLlmDialog = true">
        <el-icon><i class="ri-add-line"></i></el-icon>
        {{ $t('label.add') }}
      </el-button>

      <SettingsMenuItem
        v-for="llm in llmStore.listByType('chat')"
        :key="llm.name"
        @select="onSelected(llm.name)"
        :active="state.currentName === llm.name"
      >
        <div class="flex w-full items-center">
          <template v-if="guessChatLlm(llm)">
            <el-image
              class="border-1 h-6 w-6 rounded border-neutral-200"
              :src="`/vendor/${guessChatLlm(llm).name}.png`"
              fit="contain"
            >
              <template #error>
          ...
          <template v-if="llm.name === llmStore.defaultChatLlmName">
      ...
      <div class="flex flex-1 flex-col gap-2 overflow-auto">
        <div class="px-2 py-4">
          <div class="flex">
            ...
            <el-button @click="onSetCurrentAsDefault" :disabled="state.currentName === llmStore.defaultChatLlmName">
  ...
  <AddChatLlmDialog v-model:visible="state.showAddChatLlmDialog" @added="onAdded" />
</template>
