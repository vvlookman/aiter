...
import { chatLlms } from '@/utils/llm';
...
const onSelect = async (preset) => {
  ...
  const added = await callLlmConfig(name, 'chat', protocol, options);
...
<template>
  <el-dialog v-model="visible" :title="$t('label.add')" destroy-on-close>
    <div class="max-h-50vh my-4 overflow-auto">
      <el-button class="!ml-0 mr-4 mt-4" v-for="preset in chatLlms" @click="onSelect(preset)" round :key="preset.name">
